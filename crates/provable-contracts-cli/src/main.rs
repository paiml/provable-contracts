...
fn run_command(command: Commands) -> Result<(), Box<dyn std::error::Error>> {
    match command {
        ...
        Commands::Query {
            query, contract_dir, regex, literal, case_sensitive, limit,
            obligation, min_score, min_level, depends_on, depended_by,
            unproven, score, graph, paper, proof_status, binding_info,
            binding_gaps, diff, pagerank, call_sites, violations,
            coverage_map, project, include_project, tier, class,
            all_projects, rebuild_index, binding, format, exit_code,
        } => commands::query::run(&commands::query::QueryCliParams {
            contract_dir: &contract_dir, query_str: &query, regex, literal,
            case_sensitive, limit, obligation: obligation.as_deref(),
            min_score, min_level, depends_on: depends_on.as_deref(),
            depended_by: depended_by.as_deref(), unproven, show_score: score,
            show_graph: graph, show_paper: paper, show_proof_status: proof_status,
            show_binding: binding_info, binding_gaps, show_diff: diff,
            show_pagerank: pagerank, show_call_sites: call_sites,
            show_violations: violations, show_coverage_map: coverage_map,
            project_filter: project.as_deref(),
            include_project: include_project.as_deref(), tier, class,
            all_projects, rebuild_index, binding: binding.as_deref(),
            format: &format, exit_code,
        }),
        Commands::Book {
            contract_dir,
            output,
