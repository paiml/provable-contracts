...
Qwen3-8B end-to-end verification — composing all kernel contracts into a complete model proof
...
- [qwen3-shapes-v1](qwen3-shapes-v1.md)
...
```mermaid
graph LR
    qwen3_e2e_verification_v1["qwen3-e2e-verification-v1"] --> qwen3_shapes_v1["qwen3-shapes-v1"]
...
- $Total \approx 8.19B for Qwen3-8B$
- $Embedding: 151936 * 4096 \approx 622.3M$
- $Per-layer attention: 2*(4096^2) + 2*(1024*4096) \approx 41.9M$
- $Per-layer FFN: 3 * 4096 * 12288 \approx 151.0M$
...
|---|------|----------|--------|
| 1 | invariant | Parameter count matches architecture | $P(Qwen3-8B) in [8.0B, 8.4B]$ |
...
| 5 | bound | Verification coverage at 100% | $coverage(qwen3_contracts) = 1.0$ |
...
|----|------|------------|----------|
| FALSIFY-QW3E-001 | Parameter count | Total params ≈ 8.19B | Architecture config mismatch |
| FALSIFY-QW3E-002 | FLOPs estimate | 2P FLOPs per forward token | Missing layer in FLOP count |
| FALSIFY-QW3E-003 | Memory ordering | Q4K < Q6K < F16 < F32 memory | Quantization byte formula wrong |
| FALSIFY-QW3E-004 | Throughput roofline | tok/s bounded by bandwidth and compute | Roofline formula error |
| FALSIFY-QW3E-005 | Coverage completeness | All obligations have test or proof | Missing obligation coverage |
| FALSIFY-QW3E-006 | Compositional proof structure | Each block preserves shape: shape(block_l(x)) = shape(x) | Block l breaks shape invariant |
| FALSIFY-QW3E-007 | End-to-end shape conservation | tokens -> [seq_len, 4096] -> ... -> [seq_len, 151936] | Shape break in layer composition |

## Kani Harnesses

| ID | Obligation | Bound | Strategy |
|----|------------|-------|----------|
| KANI-QW3E-001 | QW3E-INV-001 | 1 | exhaustive |
| KANI-QW3E-002 | QW3E-ORD-001 | 4 | bounded_int |
| KANI-QWEN3_-003 | Parameter count matches architecture | 8 | exhaustive |
| KANI-QWEN3_-004 | FLOPs bounded by 2P | 8 | stub_float |
| KANI-QWEN3_-005 | Quantization memory ordering | 8 | exhaustive |
| KANI-QWEN3_-006 | Throughput increases with bandwidth | 8 | exhaustive |
| KANI-QWEN3_-007 | Verification coverage at 100% | 8 | exhaustive |
| KANI-QWEN3_-008 | Compositional proof structure | 8 | exhaustive |
| KANI-QWEN3_-009 | End-to-end shape: tokens in -> logits out | 8 | exhaustive |

## QA Gate

**Qwen3 End-to-End Verification** (F-QW3E-001)
